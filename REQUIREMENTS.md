...
| FR-6.2b | The fade's resolution in the BAR is the cell count, because the band is the top tenth of it: 11 cells of a 103-cell bar, 5 of 40, and 1 at `BAR_MIN`, where it is a single step rather than a fade. The rate limit gauges are fixed at 10 cells so their band is 2 and was never a fade. The printed NUMBER is smooth at every width regardless, because it is coloured from the real percentage rather than from a cell's position, so on a narrow pane the fade lives in the counts and the bar steps. This is FR-6.4's quantisation reaching the alarm band, and it means a judgement of how the fade looks is only good at the width it was made at. | [D] |
...
| FR-4.9 | Whether the state file should say what a session is DOING, not only what it has spent. It carries the context figures, the model and the cwd, all of which describe consumption; nothing in it distinguishes a session waiting on a person from one with nothing to do, and those look identical from outside while meaning opposite things. Raised from silo 2026-08-27, which hit the gap twice from the coordinating side: once reading a producer and a consumer as comparable because both were a percentage, and once reading a session that had finished its handoff and was waiting as idle. Not work today, and NOT a segment: the render sees one payload for one session and has no view of a board. If anything is built it is a reader over these files, and this row exists so the requirement is found before the reader is designed rather than after. | [?] |
