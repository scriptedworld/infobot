...
| FR-1.11r | A string value comes back the bytes it went in as, or the write fails loudly. Every code point a strict reader rejects or silently alters is escaped: C0, DEL, and C1. The double-quoted style already carries the full escape set on one line, so this needs no change of form and alters no byte of any value not holding one. Unescaped, `\n` and `\r` and U+0085 each come back as a space, which no reader can detect; the rest make a file no parser will read, which is the loud failure this keeps. | [A/D] |
