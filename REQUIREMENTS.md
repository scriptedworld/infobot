...
| FR-1.11d | 2026-08-27 | Canonical YAML emitted by hand rather than by importing wrench, justified by the render's runtime staying an answer. The Go port links wrench's Go pack, so the duplication dissolves rather than being defended. Retired on the measurement task 14 asked for: wrench's Go pack reproduces its `a-hand-emitted-status-file` fixture byte for byte, and infobot's emitter still produces that same fixture, checked from both ends 2026-08-27. The form itself is still required, by FR-1.11g. |
| FR-1.11n | 2026-08-27 | The form pinned independently at both ends, infobot's output being a fixture in wrench and infobot asserting its own canonical form. It existed only because two programs emitted the same bytes by agreement rather than by sharing code. Retired on the same measurement as FR-1.11d. **What no longer holds, and it is not nothing.** Its retirement was justified by a future state: the Go port links wrench's pack and there is one emitter. Until that lands there are still two, and nothing now makes a form change a two-repository change, so wrench's fixture can drift from infobot silently. FR-1.11o replaces the half that faces the file's readers; the half that faced wrench is deliberately gone and is task 14's to close by linking the library. |
