...
| FR-1.11p | infobot asserts the exact bytes it emits, against the pack that emits them. This was a mutual pin between two emitters until 2026-09-03, when the Go port linked wrench and the second emitter stopped existing; what it guards now is one emitter and a conformance check, which is the stronger of the two. The assertion stays here rather than moving to wrench, because a check needing a sibling repository present fails for the wrong reason on a fresh clone, and because these bytes are infobot's contract with its readers rather than wrench's with its consumers. A change in wrench's canonical form therefore surfaces here as a failing test rather than as a silent difference, and is announced under FR-1.11o. | [A/D] |
...
| FR-1.11d | 2026-08-27 | Canonical YAML emitted by hand rather than by importing wrench, justified by the render's runtime staying an answer. Retired 2026-08-27 on a measurement that the Go pack reproduces infobot's bytes, expecting the port to link it. The port kept the hand-emitter and wrench declined the link on 2026-08-28, so the emitter this row described was what infobot did for a fortnight after retirement. **That ended on 2026-09-03**: the Go port links wrench's pack, the hand emitter is deleted, and this row's subject no longer exists in any form. It is not revived: what it required is now FR-1.11g for the form and FR-1.11q for the spelling of a number. |
| FR-1.11n | 2026-08-27 | The form pinned independently at both ends, infobot's output being a fixture in wrench and infobot asserting its own canonical form. Retired on the same measurement as FR-1.11d, justified by a future state: the port links wrench's pack and there is one emitter. **That state did not arrive on time.** The port kept the hand-emitter and wrench declined the link on 2026-08-28, so for a fortnight two emitters was the answer; the guarantee was restored under FR-1.11p, a new id because a retired one is never reused, with FR-1.11o carrying the half facing the file's readers. The link was taken on 2026-09-03 and the state this row was retired on has now arrived, six days late. **Retiring a row on a future state is still the mistake here**, and the arrival does not redeem it: it recorded a live gap as a closed one, and which way it eventually resolved was never the point. |
